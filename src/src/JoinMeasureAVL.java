 ...
 *
 */
public class JoinMeasureAVL {
    IAVLNode root;
    int size;
    IAVLNode min;
    IAVLNode max;

    public JoinMeasureAVL() {
    ...
    private List<Integer> split(int x, IAVLNode xNode) {
        List<Integer> cost = new ArrayList<>();
        JoinMeasureAVL[] result = new JoinMeasureAVL[2];
        // smaller
        result[0] = new JoinMeasureAVL();
        // bigger
        result[1] = new JoinMeasureAVL();
        ...
        while (node != null) {
            ...
            if (node.getKey() < x) {
                ...
                JoinMeasureAVL smaller = new JoinMeasureAVL();

                smaller.root = node.getLeft();

                smaller.root.setParent(null);
                cost.add(smaller.join(middle, result[0]));
                result[0] = smaller;
            } else {
                IAVLNode middle = this.duplicateNode(node);
                JoinMeasureAVL bigger = new JoinMeasureAVL();
                bigger.root = node.getRight();
                bigger.root.setParent(null);

                cost.add(result[1].join(middle, bigger));
            }
    ...
    private void setFields(JoinMeasureAVL tree) {
    ...
    public int join(IAVLNode x, JoinMeasureAVL t) {
        ...
        JoinMeasureAVL tallerTree = t;
        JoinMeasureAVL shorterTree = this;
